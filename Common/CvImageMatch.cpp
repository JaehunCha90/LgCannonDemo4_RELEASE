                    ...
                    if (maxCosine < 0.3)
                    {
                        ...
                        if ((near1 > 0.75) && (near1 < 1.25))
                        {
                            ...
                            if (corners.size() != 0)
                            {
                                ...
                                for (int k = 0; k < NUM_SIGNS; k++)
                                {
                                    bitwise_xor(new_image, symbols[k].img, diffImg,
                                        noArray());
                                ...
                                if ((match != -1) && (minDiff < MIN_DIFF_MATCH))
                                {
                                    ...
                                    {
                                        bool found = false;
                                        for (int q = 0; q < NumMatches && q < MAX_DETECTED_MATCHES && !found; ++q)
                                        {
                                            double distance = cv::norm(center - DetectedMatches[q].center);
                                            if (distance < 5.00)
                                            {
                                                if (DetectedMatches[q].Diff > minDiff)
                                                {
                                                    if (DetectedMatches[q].match != match)
                                                        printf("Target Change\n");
                                                    DetectedMatches[q].center = center;
                                                    DetectedMatches[q].Diff = minDiff;
                                                    DetectedMatches[q].match = match;
                                                    DetectedMatches[q].xmin = corners[0].x;
                                                    DetectedMatches[q].xmax = corners[2].x;
                                                    DetectedMatches[q].ymin = corners[0].y;
                                                    DetectedMatches[q].ymax = corners[2].y;
#if DEBUG
                                                    printf("Updated %d\n", match);
#endif
                                                }
...
void WriteFile(const  char* path, const char* folder, const char* filename, char * ext, const cv::Mat src)
{
    ...
    snprintf(PathFilename_xml, sizeof(PathFilename_xml), "%s%s/%s.xml", path, folder, filename);
    snprintf(PathFilename_jpg, sizeof(PathFilename_jpg), "%s%s/%s.%s", path, folder, filename, ext);
#if DEBUG
    printf("%s\n", PathFilename_jpg);
#endif
#if  defined(_WIN32) || defined(_WIN64)
